...
describe(require('../_lib/test-helper').testName(__filename, 3), function() {
  ...
  it('starts the cluster internal first, connects a client to the local instance, and is able to access the remote component via the broker', function(done) {
    ...
    startEdge(1, 1)
      .then(() => {
        let child = fork(libDir + 'test-24-sub-process.js', ['2']);
      ...
      .then(() => {
        return fork(libDir + 'test-24-sub-process.js', ['3']);
      })
  ...
  function localInstanceConfig(seq, sync, dynamic) {
    ...
    let brokerComponentPath = libDir + 'integration-24-broker-component';
    config.modules = {
      brokerComponent: {
