...
describe(testHelper.testName(__filename, 3), function() {
  ...
  it('test persistMembers', function(done) {
    startProcess('hosts=127.0.0.1:58000 persistMembers=true membername=seedNode')
      ...
      .then(() => {
        return startProcess('hosts=127.0.0.1:58000 persistMembers=true membername=seedNode');
      })
      .then(node => {
        seedNode = node;
        return Promise.delay(5000);
    ...
    startProcess('hosts=127.0.0.1:58000 membername=seedNode')
      ...
      .then(function(node) {
        ...
        setTimeout(function() {
          ...
          startProcess('hosts=127.0.0.1:58000 membername=seedNode').then(function(node) {
            seedNode = node;
            setTimeout(function() {
              expect(messages.member1[0]).to.eql(undefined);
