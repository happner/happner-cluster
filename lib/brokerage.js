...
Brokerage.prototype.__handleDependencyMet = function(whatChanged) {
  ...
  this.__injectedElements
    .filter((injectedComponent) => {
      return injectedComponent.component.name == whatChanged.componentName && whatChanged.countMatches > 0;
    })
...
Brokerage.prototype.__updateInjectedComponent = function(changedComponent, whatChanged) {
  ...
  if (!whatChanged.description) return;
  let whatChangedDescription = Array.isArray(whatChanged.description)?whatChanged.description.pop():whatChanged.description;
  let description = whatChangedDescription.components[changedComponent.component.name];
  if (whatChangedDescription.self) return;
  let newModel = {};
  newModel[changedComponent.component.name] = description;
  let newAPI = this.__client.construct(newModel);
  let updatedElement = this.constructBrokeredElement(changedComponent.component.name, description, newAPI, true);
