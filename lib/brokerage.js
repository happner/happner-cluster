...
Brokerage.prototype.__handlePeerDeparted = function(peer) {
  ...
  this.__injectedElements
    ...
    .forEach((changedComponent, changedComponentIndex, changedComponents) => {
      changedComponent.module.instance.disconnect();
      //remove this component from injected elements, but only if it is not the last one
      //otherwise our exchange api contract will change and clients will break
      if (changedComponents.length > 1) this.__injectedElements.splice(1, changedComponentIndex);
    });
};
...
Brokerage.prototype.__updateInjectedComponent = function(changedComponent, whatChanged) {
  ...
  let updatedElement = this.constructBrokeredElement(changedComponent.component.name, whatChanged.description, newAPI, whatChanged.url, true, whatChanged.meshName);
...
Brokerage.prototype.__updateInjectedElements = function(changedComponent, updatedElement){
  //this is a 'place holder' preconfigured component, do not remove it
  if (!changedComponent.meshName)
    return this.__injectedElements.splice(this.__injectedElements.indexOf(changedComponent), 1, updatedElement);
...
Brokerage.prototype.constructBrokeredElement = function(brokeredComponentName, model, $happn, url, dynamic, meshName){
  return {
    url,
    meshName,
    module: {
      ...
      config: {
        instance:  require('./broker').create(brokeredComponentName, model, $happn, this.__mesh, url, dynamic)
      }
    },
    component: {
      name: brokeredComponentName,
