...
module.exports.create = require('bluebird').promisify(function (config, callback) {
  ...
  Happner.create(config)
    ...
    .then(function () {
      ...
      if (brokerage && config.cluster.dependenciesSatisfiedDeferListen) return brokerage.deferProxyStart(happner._mesh.happn.server.services.proxy);
      return happner._mesh.happn.server.services.proxy.start();
    })

    .then(function () {
      let brokerWebProxy = require('./broker-web-proxy').instance(config.name);
      if (brokerWebProxy) return brokerWebProxy.attachToClusterMiddlewareServer(happner._mesh.happn.server.connect);
