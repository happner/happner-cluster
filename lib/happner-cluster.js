...
module.exports.create = require("bluebird").promisify(function(
  config,
  callback
) {
  var happner, cursor;
  ...
  if (cursor.config.replicate == null) cursor.config.replicate = [];

  //replicate events cluster-wide
  ...
  Happner.create(config)
    ...
    .then(function() {
      ...
      if (brokerage && config.cluster.dependenciesSatisfiedDeferListen)
        return brokerage.deferProxyStart(
          happner._mesh.happn.server.services.proxy
        );
    ...
    .then(function() {
      ...
      if (brokerWebProxy)
        return brokerWebProxy.attachToClusterMiddlewareServer(
          happner._mesh.happn.server.connect
        );
