...
module.exports = function (message, recipients, callback) {
  ...
  recipients.forEach(function (subscription) {
    ...
    try {
      ...
      if (subKeys.length == 0) {
        // Bug in happn-3 resubscribe after server restart was resulting in
        // an empty subscriptionData object, this was then filtering out
        // the subscriber. The bug is fixed in happn-3 but this check is
        // done anyway in case a similar but returns.
        subscription.__keep = true;
        return;
      }
