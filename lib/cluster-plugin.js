          ...
          Object.keys(mesh._mesh.elements).forEach(componentName => {
            ...
            if (dependencies['$broker']) {
              ...
              brokeredModels[componentName] = {
                package: dependencies['$broker'],
                $happn: $happn
              };
              client.construct(dependencies['$broker'], $happn);
              return;
            }
            // amend $happn with model
            client.construct(dependencies, $happn);
            client.on(`${componentName}/startup/dependencies/satisfied`, () => {
              $happn.emitEvent('startup/dependencies/satisfied', componentName);
            });
          });
