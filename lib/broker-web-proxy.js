...
module.exports = class BrokerWebProxy {
  ...
  __onProxyError(err, req, res) {
    this.logger.warn(
      `proxy error occurred for url: ${req.url}, error: ${err.message}`
    );
    res.writeHead(500);
    res.end();
  }
  ...
  selectTarget(peers, req) {
    let requestToken =
      this.mesh._mesh.happn.server.services.security.tokenFromRequest(req) ||
      Date.now();
  ...
  connectRoute(path, target) {
    ...
    this.logger.info(
      `added route ${target.url}, route handlers count: ${this.routes[path].size}`
    );
  }
